...
const getDestIcon = (dest: string) => {
  ...
  if (
    ["t1", "t2", "t3", "t4", "aeropuerto", "airport"].some((k) => d.includes(k))
  )
    return E.avion;
  if (["hotel"].some((k) => d.includes(k))) return E.hotel;
  if (
    ["estacion", "tren", "atocha", "chamartin", "station", "train"].some((k) =>
      d.includes(k),
    )
  )
    return E.tren;
  if (
    ["restaurante", "cena", "comida", "restaurant", "dinner", "lunch"].some(
      (k) => d.includes(k),
    )
  )
    return E.restaurante;
  if (
    ["oficina", "empresa", "trabajo", "office", "company", "work"].some((k) =>
      d.includes(k),
    )
  )
    return E.oficina;
...
const buildWhatsAppUrl = (data: FormData) => {
  ...
  if (data.extras && data.extras !== "ninguno")
    lines.push(`${E.bebe} *Accesorios:* ${data.extras}`);
  if (data.comentarios)
    lines.push(`${E.nota} *Comentarios:* ${data.comentarios}`);
  return `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(lines.join("\n"))}`;
};

const selectClass =
  "w-full h-12 rounded-xl border border-border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary appearance-none";
...
let googlePlacesLoader: Promise<PlacesLibrary> | null = null;
...
function loadGooglePlaces(apiKey: string) {
  ...
  googlePlacesLoader = new Promise<PlacesLibrary>((resolve, reject) => {
    const finishLoading = () => {
      ...
      if (loadedGoogle.maps.importLibrary) {
        loadedGoogle.maps.importLibrary("places").then(resolve).catch(reject);
        return;
      }

      if (loadedGoogle.maps.places?.Autocomplete) {
        resolve(loadedGoogle.maps.places);
        return;
      }
    ...
    if (existingScript) {
      if (
        (window as GoogleWindow).google?.maps?.importLibrary ||
        (window as GoogleWindow).google?.maps?.places?.Autocomplete
      ) {
        finishLoading();
        return;
      }
    ...
    const setupAutocomplete = async () => {
      try {
        ...
        if (originInputRef.current) {
          ...
          listeners.push(
            autocomplete.addListener("place_changed", () =>
              setPlaceValue("origen", autocomplete),
            ),
          );
        }
        ...
        if (destinationInputRef.current) {
          ...
          listeners.push(
            autocomplete.addListener("place_changed", () =>
              setPlaceValue("destino", autocomplete),
            ),
          );
        }
      } catch (error) {
    ...
    <div className="mx-auto max-w-5xl rounded-3xl border border-black/10 bg-white p-5 shadow-[0_24px_70px_rgba(0,0,0,0.18)] sm:p-8 md:p-10">
      <div className="mb-7 hidden flex-col gap-2 border-b border-black/10 pb-6 sm:flex-row sm:items-end sm:justify-between md:flex">
        <div>
          ...
          <h3 className="font-sans text-2xl font-bold tracking-tight md:text-3xl">
            {t("form_title")}
          </h3>
        </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            >
              <option value="" disabled>
                {t("form_time_placeholder") || "Selecciona hora"}
              </option>
              {TIME_SLOTS.map((slot) => (
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-6">
          <div className="space-y-2">
            <Label htmlFor="pasajeros" className="text-sm font-medium">
              {t("form_passengers")}
            </Label>
            <div className="relative">
              ...
              <button
                ...
                onClick={() =>
                  form.setValue(
                    "pasajeros",
                    Math.max(1, Number(form.getValues("pasajeros")) - 1),
                    { shouldValidate: true },
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  form.setValue(
                    "pasajeros",
                    Math.min(8, Number(form.getValues("pasajeros")) + 1),
                    { shouldValidate: true },
                  )
                }
            ...
            {form.formState.errors.pasajeros && (
              <p className="text-primary text-xs mt-1">
                {form.formState.errors.pasajeros.message}
              </p>
            )}
          </div>
          <div className="space-y-2">
            <Label htmlFor="equipaje" className="text-sm font-medium">
              {t("form_luggage")}
            </Label>
            <div className="relative">
              ...
              <button
                ...
                onClick={() =>
                  form.setValue(
                    "equipaje",
                    Math.max(0, Number(form.getValues("equipaje")) - 1),
                    { shouldValidate: true },
                  )
                }
                className="absolute left-0 top-0 flex h-12 w-10 items-center justify-center text-muted-foreground transition-colors hover:text-primary"
              ...
              <button
                ...
                onClick={() =>
                  form.setValue(
                    "equipaje",
                    Math.min(10, Number(form.getValues("equipaje")) + 1),
                    { shouldValidate: true },
                  )
                }
                className="absolute right-0 top-0 flex h-12 w-10 items-center justify-center text-muted-foreground transition-colors hover:text-primary"
            ...
            {form.formState.errors.equipaje && (
              <p className="text-primary text-xs mt-1">
                {form.formState.errors.equipaje.message}
              </p>
            )}
          </div>
        </div>
        ...
        {showAdditional && (
          <div className="space-y-5">
            {/* Vuelo / Tren */}
            <div className="space-y-2">
              <Label htmlFor="vuelo" className="text-sm font-medium">
                {t("form_flight")}
              </Label>
              <div className="relative">
